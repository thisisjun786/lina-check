...
`NODE_TEST_` 로 시작하는 이름은 접두사로 막는다. `NODE_TEST_CONTEXT` 는 이름으로 적혀 있었는데
`NODE_TEST_WORKER_ID` 는 없었다. 이 네임스페이스는 Node 가 소유하고 늘릴 수 있으므로 하나씩 적는
방식으로는 같은 실수가 반복된다. 파생 테스트는 자기가 쓴 fixture 를 읽지 러너의 장부를 읽지 않는다.
...
`globalThis` 와 `global` 도 같다. 속성 읽기로만 닿을 수 있고 그 밖의 자리는 전부 거부한다.
이름에 담는 것도, 괄호로 묶는 것도(`(globalThis)`), 쉼표식도(`(0, globalThis)`), 함수에 넘기는
것도 거부다. 한동안 인자 자리를 예외로 뒀는데, 그건 파생 response-deadlines 테스트의
`t.mock.method(globalThis, "fetch", ...)` 한 철자 때문이었다. 그 테스트를 다른 파생 스위트가 이미
쓰는 저장·복원 형태(`globalThis.fetch` 를 읽고 `t.after` 에서 되돌린다)로 바꿔서 예외를 없앴다.
예외가 없으니 괄호와 호출을 구분할 필요도 없다.

`process.env` 의 점 접근도 아무 멤버나 되는 게 아니라 환경 변수 이름이어야 한다.
`process.env.valueOf()` 는 객체 전체를 돌려주고, 그걸 열거하면서 조각으로 조립한 이름과 비교할 수
있다. 이 저장소가 읽는 변수는 전부 대문자 형태다.
...
`Object.getOwnPropertyDescriptor` 단수형도 막는다. 키가 멤버가 아니라 인자라서 위 규칙이 못 본다.
복수형(`...Descriptors`)은 막지 않는다. 키를 받지 않고, 업스트림 핀이 걸린 하네스가 쓰며, 그
결과에서 생성자에 닿으려면 어차피 아래 경계 절의 계산된 읽기가 필요하다.
