...
`globalThis` 도 같은 이유로 묶는다. 대괄호 접근은 거부하고, 점 접근은 허용하고, 그 밖의 자리에서는
인자로 넘기는 형태만 허용한다(`t.mock.method(globalThis, "fetch", ...)` 가 이 저장소에서 쓰는
유일한 형태다). `const root = globalThis` 처럼 이름에 담는 것은 거부한다. 담기고 나면 계산된
읽기가 이 규칙이 보지 않는 이름에서 계속된다.

인자로 넘긴 값을 받은 함수가 계산된 키로 읽는 형태는 남는다. 그건 아래 경계 절의 잔여물과 같은
부류이고, 임의 값에 대한 계산된 **읽기** 를 전부 막으면 업스트림 핀이 걸린 하네스가 쓰는 인덱스
읽기까지 막혀 회복 스위트를 잃는다.
