...
### 23라운드 — 이름으로 막고, import.meta 도 목록으로

세 건이 왔고 둘은 같은 것이다.

Codex 와 Devin 이 각각 `Function` 호출 철자를 짚었다. 전역 `Function` 은 `new` 없이 불리고,
호출 앞에 공백이 올 수 있고, 둘 다 바인딩에 먼저 담을 수 있다. 19라운드에서 막은 것은 호출 모양
두 개뿐이라 공백 하나로 빠져나간다. 22라운드에서 `.constructor` 를 막아 우회 경로는 닫혔지만
직통은 열려 있었다. 이제 호출 모양이 아니라 이름을 막는다.
`eval`·`Function`·`AsyncFunction`·`GeneratorFunction` 네 전역이고, closure 에는 한 번도
나오지 않는다.

Codex 의 세 번째 건은 `import.meta.main` 이다. 레인이 `node --test` 로 파일을 띄우면 참이고
관측이 직접 import 하면 거짓이다. 금지 신호를 하나도 적지 않고 어느 실행인지 알려 주는 값이라
13라운드에서 정한 방식이 그대로 적용된다. process 와 같은 모양으로 허용 속성 목록을 뒀고, 거기
있는 것은 `url` 하나다. closure 에서 실제로 쓰이는 것도 `url` 11회뿐이다.
...
### 26라운드 — 계산된 호출과 node:module named import

세 건. 둘은 수정, 하나는 답만 했다.

`new Script("...")["run" + "InThisContext"]()` 는 허용된 객체를 금지된 동사로 되돌린다. 25라운드
까지의 답은 "계산된 키 접근은 스캔이 결정할 수 없다" 였는데, 그 문장이 너무 넓었다. 결정할 수 없는
것은 인덱스 **읽기** 다. 계산된 이름으로 하는 **호출** 은 다르다. closure 에 하나도 없고, 허용된
객체를 다시 위험하게 만드는 것이 정확히 그 형태다. 그래서 그것만 막는다. `rows[i]` 와
`scripts[name]` 은 그대로 돌고 리터럴 키 호출도 통과한다.

node:module 의 named import 는 `createRequire` 만 읽고 나머지는 암묵 허용이었다. `register` 는
커스터마이제이션 훅을 설치하고 그 훅은 이 isolate 밖에서 돈다. 관측의 `child_process` 계측이 닿지
않는 곳이라 "기동 0건" 이 거짓이 될 수 있었다. 네임스페이스에 쓰던 같은 목록을 named import 에도
적용한다.

거부 대조 넷과 허용 대조 하나를 더해 파생 계약 대조가 123에서 128이 됐다.

세 번째는 Devin 의 정보성 지적이다. 외부 천장이 경로를 해시하므로 모듈 내용이 바뀌어도 천장은
그대로다. 맞고, 의도한 것이다. 천장이 묶는 것은 어느 모듈에 닿을 수 있는지이고, 그 모듈의 내용은
빌드와 업스트림 핀이 따로 본다. 내용까지 묶으면 제품 코드를 고칠 때마다 이 목록을 갱신해야 하는데,
그건 이 검사가 답하려는 질문이 아니다.

### 27라운드 — 스캐너가 스스로 지우고 있었다

세 건. 하나는 스캐너 자체의 버그이고 둘은 앞 라운드가 만든 자리다.

Devin 이 `codeOnly` 의 보간 처리를 짚었다. 보간의 깊이를 원시 중괄호로 세고 있어서, 보간 안의
문자열이나 주석에 들어 있는 `}` 가 깊이를 먼저 0으로 만든다. 그 뒤의 코드는 템플릿 텍스트로
비워진다. 스캐너가 읽어야 할 코드를 스스로 지우고 있었다는 뜻이고, 이 PR 의 다른 모든 규칙이 그
사본을 읽으므로 전부 영향을 받는다. 이번 라운드에서 제일 값어치 있는 지적이다.

구조별 프레임을 쌓는 방식으로 고쳤다. 보간은 코드 프레임을 밀고, 문자열·템플릿·주석·정규식은 각자
프레임에서 처리되므로 그 안의 중괄호는 보간을 끝내지 못한다. 중첩 템플릿도 따라온다. 대조는
보간 안에 `process.report` 를 숨긴 소스를 먹인다. 옛 계산법에서는 통과하고 지금은 거부된다.

Codex 의 두 건. 하나는 26라운드 규칙이 대괄호 뒤의 괄호만 봤다는 것이다.
`const { ["run" + "InThisContext"]: go } = script` 는 메서드를 먼저 꺼내고 나중에 평범한 이름으로
부른다. 구조 분해 패턴을 객체 리터럴과 뒤따르는 대입으로 구분해 막았다. 하네스가 만드는
`{ [item.key]: item }` 은 값을 만드는 쪽이라 통과한다.

다른 하나는 `new Error().stack` 이다. 관측이 만든 러너 경로가 스택에 들어가고 레인에는 그쪽 진입
프레임이 들어간다. 13라운드 방식 그대로 읽기를 막았다. `.stack`·`captureStackTrace`·
`prepareStackTrace` 다.

거부 대조 다섯과 허용 대조 하나를 더해 파생 계약 대조가 128에서 133이 됐다.

### 28라운드 — 대조가 지목한 자리를 고치는가

한 건이고, 이번에는 탐지기가 아니라 인도된 증거 자체의 결함이다.

Devin 이 실패 대조의 앵커를 짚었다. `String.replace` 는 첫 일치만 바꾸는데 close-policy 대조의
앵커 `.github/workflows/sweep.yml.disabled` 는 그 파일에 두 번 나온다. 대조가 지목하지 않은 자리를
변조하고, 거기서 난 실패도 탐지로 세어진다. 영수증이 돌지도 않은 단언에 대해 민감도를 주장할 수
있었다는 뜻이다.

앵커는 이제 정확히 한 번 일치해야 하고 아니면 대조가 시작하지 않는다. close-policy 앵커에는 그것이
속한 테스트 이름을 붙여 유일하게 만들었다. 대조 열한 개를 다시 돌려 전부 탐지됐고 영수증을 다시
썼다. selftest 도 "있는가" 대신 "정확히 한 번인가" 를 열한 개 전부에 대해 단언한다.

### 29라운드 — 운영체제가 가진 기록

한 건. `/proc/self/cmdline` 에는 관측의 러너 경로가, 레인에는 `node --test` 의 인자가 들어간다.
읽는 쪽은 허용 목록에 있는 `node:fs` 라서 모듈로는 막히지 않는다. 그래서 읽는 대상을 막았다.
슬래시 두 개로 맞춘다. `/process...` 로 시작하는 정규식이 같은 네 글자로 시작하고 파생
close-policy 테스트에 하나 있어서, 그 음성 대조도 같이 넣었다. 거부 하나와 허용 하나를 더해 파생
계약 대조가 133에서 135가 됐다.

여기서 이 PR 의 탐지기 강화를 멈춘다. 20라운드부터 열 번 연속으로 새 회피 경로가 왔고 전부
처리했지만, 남은 것들은 성격이 같다. 조각으로 조립한 문자열과 임의 객체에 대한 계산된 접근이다.
텍스트 스캔이 결정할 수 없는 자리이고, 그 경계는 `030_detectors.md` 에 적혀 있다. 이 뒤에 오는
강화 제안은 후속 항목으로 기록하고 이 PR 에서는 받지 않는다. 인도된 커버리지 자체의 결함,
즉 회복이라고 적은 케이스가 실제로 검사하지 않거나 대응표 수치가 틀리거나 게이트가 실패하는
경우는 예외다.

### 30라운드 — 접두사로 닫았다

한 건이고 한 줄짜리다. Node 24 는 레인의 `node --test` 에서 `NODE_TEST_WORKER_ID` 도 넣는다.
`NODE_TEST_CONTEXT` 는 16라운드에 이름으로 적었는데 이건 없었다.

이름을 하나 더 적는 대신 접두사로 막았다. `NODE_TEST_` 로 시작하는 이름은 전부 거부한다. 이
네임스페이스는 Node 가 소유하고 늘릴 수 있어서, 하나씩 적는 방식이 이 파일이 반복해 온 실수의
모양이다. 대조 하나를 더해 파생 계약 대조가 135에서 136이 됐다.

29라운드에 적은 정지 규칙은 그대로다. 이건 새 부류가 아니라 이미 갖고 있던 목록의 누락이라
처리했다.

### 31라운드 — globalThis 를 이름에 담는 것

한 건. 30라운드까지 `globalThis` 는 바로 뒤의 대괄호만 봤다. `const root = globalThis;` 뒤에
`root["pro" + "cess"]` 로 이어가면 그 이름은 더 이상 감시 대상이 아니다.

점 접근은 허용하고, 대괄호는 거부하고, 그 밖에서는 인자로 넘기는 형태만 허용한다.
`t.mock.method(globalThis, "fetch", ...)` 가 이 저장소가 쓰는 유일한 형태이고, 이름에 담는 것은
거부한다. 거부 대조 둘을 더해 파생 계약 대조가 136에서 138이 됐다.

인자로 받은 값을 계산된 키로 읽는 간접 형태는 남는다. 임의 값에 대한 계산된 읽기를 전부 막으면
업스트림 핀이 걸린 하네스의 인덱스 읽기까지 막혀 회복 스위트를 잃는다. 29라운드에 적은 잔여물과
같은 부류이고 `030_detectors.md` 에 적었다.

### 32라운드 — 예외를 없앴다

세 건. 둘은 31라운드가 남긴 인자 예외를 가리켰고 하나는 `process.env` 의 멤버다.

Devin 은 `(globalThis)` 가 괄호 때문에 인자로 읽힌다고 했고, Codex 는 인자를 받은 함수가 계산된
읽기를 대신할 수 있다고 했다. 둘 다 맞다. 예외가 있는 한 그 예외를 어떻게 흉내 내느냐의 문제가
계속 남는다.

그래서 예외를 없앴다. `globalThis` 와 `global` 은 속성 읽기로만 닿을 수 있다. 예외가 있던 이유는
파생 response-deadlines 테스트의 `t.mock.method(globalThis, "fetch", ...)` 한 철자였고, 그 테스트를
다른 파생 스위트가 이미 쓰는 저장·복원 형태로 바꿨다. `globalThis.fetch` 를 읽어 두고 `t.after`
에서 되돌린다. 열 케이스 전부 그대로 통과하고, 실패 대조 영수증은 그 스위트의 바이트가 바뀌었으므로
다시 썼다(`suites=10 controls=11 detected=11 undetected=0`).

Codex 의 세 번째 건은 `process.env.valueOf()` 다. 점 접근을 전부 허용하고 있었는데, 그 호출은
환경 객체 전체를 돌려준다. 열거해서 조각으로 만든 이름과 비교하면 금지 신호를 하나도 적지 않고
읽을 수 있다. 이제 `process.env.` 뒤는 환경 변수 이름이어야 한다. 이 저장소가 읽는 여섯 개는 전부
대문자 형태다.

거부 대조 넷을 더하고 허용 대조 하나를 새 철자로 바꿔 파생 계약 대조가 138에서 142가 됐다.

### 33라운드 — 디스크립터와 Windows

두 건. 하나는 수정하고 하나는 후속 항목으로 남긴다.

Codex 가 `Object.getOwnPropertyDescriptor(prototype, "con" + "structor").value` 를 짚었다.
22라운드의 리플렉션 차단은 멤버 접근만 봤는데 이건 키가 인자다. 단수형을 막았다. 복수형은 막지
않는다. 키를 받지 않고, 업스트림 핀이 걸린 하네스가 한 번 쓰며, 그 결과에서 생성자에 닿으려면
어차피 계산된 읽기가 필요하다. 거부 대조 하나와 허용 대조 하나를 더해 파생 계약 대조가 142에서
144가 됐다.

Devin 의 건은 Windows 에서 `reapLaunchGroup` 이 분리된 프로세스 그룹을 정리하지 못한다는 것이다.
맞는 관찰이지만 이 PR 에서 고치지 않는다. 고치려면 레인 러너가 `taskkill` 류를 띄워야 하는데, 그건
안전 레인의 동작 변경이고 이 호스트에서 검증할 수 없는 플랫폼이다. 회복된 커버리지의 결함도
아니다. 후속 항목으로 기록한다.

## 현재 값 — 20~33라운드 head 기준
...
25라운드 head `7c3e7660` 에서도 여덟 게이트 exit 0, 레인 132초이고 값은
`derivedTestContract=123` 이다. 26라운드 head `aaecb989` 에서도 여덟 게이트 exit 0, 레인 134초
이고 값은 `derivedTestContract=128` 이다. 27라운드 head `6c66d391` 에서도 여덟 게이트 exit 0,
레인 134초이고 값은 `derivedTestContract=133` 이다. 28라운드는 탐지기 수치를 바꾸지 않고 실패
대조 영수증을 다시 썼다(`suites=10 controls=11 detected=11 undetected=0`, head `01d6379b`,
여덟 게이트 exit 0, 레인 133초). 29라운드 head `f6add2ba` 에서도 여덟 게이트 exit 0, 레인 133초
이고 값은 `derivedTestContract=135` 다. 30라운드 head `3d235130` 에서도 여덟 게이트 exit 0,
레인 132초이고 값은 `derivedTestContract=136` 이다. 31라운드 수정 뒤 값은
`derivedTestContract=138` 이다(head `c6748f53`, 여덟 게이트 exit 0, 레인 133초). 32라운드 수정 뒤
값은 `derivedTestContract=142` 다(head `5c3ccc33`, 여덟 게이트 exit 0, 레인 133초). 33라운드 수정
뒤 값은 `derivedTestContract=144` 이고 나머지는 같다. 그 head 에서도 여덟 게이트를 다시 돌린다.
문서가 자기 커밋의 SHA 를 담을 수는 없으므로 마지막 head 번호는 PR 본문과 인도 보고에 적는다.

### 후속 항목 후보

이 PR 에서 받지 않고 기록만 하는 것들이다.

| 항목 | 출처 | 왜 여기서 안 하나 |
| -- | -- | -- |
| Windows 에서 분리된 프로세스 그룹 정리 | Devin, 33라운드 | 안전 레인의 동작 변경이고 이 호스트에서 검증 불가 |
| 임의 값에 대한 계산된 **읽기** | Codex·Devin, 22·31·32라운드 | 업스트림 핀이 걸린 하네스의 인덱스 읽기까지 막혀 회복 스위트를 잃는다 |
| 조각으로 조립한 문자열 일반 | Codex, 29라운드 | 텍스트 스캔이 결정할 수 없다. 리뷰가 받는 자리로 적어 뒀다 |
| 외부 천장에 모듈 내용까지 묶기 | Devin, 27라운드 | 제품 코드를 고칠 때마다 갱신해야 해서 읽지 않고 갱신하는 목록이 된다 |
| 파서 기반 분석 | Devin·Codex, 여러 라운드 | 이 저장소는 파서 의존성을 새로 들이지 않는다 |

대응표는 그대로다. 열네 라운드 모두 탐지기와 대조만 건드렸고 회복 범위는 바뀌지 않았다. 32라운드에
파생 테스트 한 곳의 철자를 바꿨지만 케이스 이름과 단언은 그대로이고 열 케이스 전부 통과한다.
