...
여덟 게이트 exit 0, 레인 133초). 29라운드 수정 뒤 값은 `derivedTestContract=135` 이고 나머지는
같다. 그 head 에서도 여덟 게이트를 다시 돌린다. 문서가 자기 커밋의 SHA 를 담을 수는 없으므로
마지막 head 번호는 PR 본문과 인도 보고에 적는다.

대응표는 그대로다. 열 라운드 모두 탐지기와 대조만 건드렸고 회복 범위는 바뀌지 않았다.
