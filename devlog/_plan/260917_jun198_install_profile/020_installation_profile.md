...
### 리뷰 2회전 정정 · 허가 방식은 두 가지다

처음 구현은 명시 허가에도 레지스트리 항목을 함께 요구했다. 이 문서에 적은 규칙에는 없던
조건이고, 그것 때문에 Worker(원격 레지스트리)와 Node(보존된 로컬 인벤토리)가 같은 설치에 대해
서로 다른 답을 냈다. 조건을 없앴다. 지금 규칙은 이렇다.

| 허가 | 필요한 것 |
| -- | -- |
| `targets.repositories` 에 이름을 적는다 | 그것뿐. 레지스트리 불필요 |
| `targets.fallback_owners` 에 소유자를 적는다 | 패턴이 레지스트리에 있으므로 `registry_url` 필요 |

소유자만 적고 레지스트리를 비우면 파싱 단계에서 거부한다(`installation-inoperable`).
깨끗하게 파싱된 뒤 모든 대상을 거부하는 것은 설정 거부가 아니라 고장으로 읽히기 때문이다.

허가와 서술을 섞지 않는다. 허가된 대상을 어떤 설정으로 다룰지는 프로필 인벤토리가 답한다.

### 파서는 스키마와 같은 문서만 받는다

2회전에서 런타임이 스키마보다 관대하다는 지적을 받았고 맞았다. 루트와 각 절에서 미선언 키를
거부하고, 필수 문자열 필드를 요구하고, 패턴 검사 전에 대소문자를 접지 않는다. 스키마 패턴이
소문자 전용이므로 먼저 접으면 스키마가 거부하는 문서를 받아들이게 된다.

`installationFromEnv` 만 목록 항목을 소문자로 접는다. Worker 설정에는 맞출 스키마가 없고,
거기서 거부하면 오류가 아니라 조용히 미설정된 설치로 나타나기 때문이다. 그 편이 더 나쁘다.
