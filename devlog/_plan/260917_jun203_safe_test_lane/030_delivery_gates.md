...
| 6 | `corepack pnpm run lina:test-safe` | 0 | 134.68 | 통과 2579건, fail 0, skip 0 |
...
- 편입한 206개의 실행 경계는 읽어서가 아니라 관측해서 정했다. 파일마다 자식 프로세스를 막은
  권한 모델 아래에서 한 번, 소켓 bind 와 나가는 요청을 기록하는 프로브 아래에서 한 번 돌렸다.
  결과는 자식 프로세스 33개, loopback 서버 4개, loopback 밖으로 나간 요청 0건이다.
  방법과 파일 목록은 `020_fixture_lane.md`
