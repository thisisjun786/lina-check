...
## 부록 D · 기록 계약 (wp3·wp4·wp5)

기록 문서의 내용을 감사했다(dispatch `jun135-audit-records-2`, 세 절 전부 FAIL).
지적은 "기록에 무엇이 반드시 들어가야 하는가"였고 전부 접는다. 아래가 구속력 있는 기록 계약이다.

### D1 · 상태를 셋으로 나눈다 (020·060)

두 상태가 아니라 세 상태다. 하나로 뭉치면 거짓이 된다.
...
"최종 head" 라고 쓰지 않는다. 측정한 SHA 를 쓴다. 나중 기록이 과거 실행을 최종 head 증거로
만들 수 없다. 최종 head 게이트 증거는 wp6 이 따로 수집한다.

측정 방법도 항목마다 적는다. 파이프라인 뒤의 `echo $?` 로 읽은 값과 파일 리다이렉트 후 읽은
값을 구분한다. 파생 상태에서 다시 돌린 결과가 원본 기준의 종료 코드를 증명하지는 않는다.
...
- 통과했다고 쓰지 않을 것. upstream 전체 테스트, 라이브 연동, 임시 파일 쓰기가 0 이라는 주장.

### D3 · 프로브 기록 (030)

- `git status --porcelain` 문자열 비교를 "바이트 동일"로 쓰지 않는다. 그 비교는 모든 내용 변경,
  무시된 파일 쓰기, 외부 부작용을 잡지 못한다.
- 커밋된 프로브 결과 파일은 **그 시점의 관측 기록**이고 기대 출력 oracle 이 아니다. 표준 출력은
  체크아웃 밖에서 받아 파일로 옮긴다. SHA·시각·명령 출처는 `030` 본문에 적고, 프로브 출력 자체에는
  시각 같은 변동 값을 넣지 않는다.
- 증거 파일을 `derived.files` 에 선언한다. 선언 파일 수가 11에서 12로 바뀌므로, 11 은 그 이전
  측정값으로 보존해 적는다.
- 증명하지 않는 것을 명시한다. 직접 node 실행 우회, 원격 Actions·App·ruleset, 기능의 부재,
  샌드박스, 그리고 "모든 실행 입구"를 다 봤다는 주장.

### D4 · 조사 결론의 한정 (040)

QA Convex 두 변수에 "이 저장소에 소비자가 없고 대상 저장소 툴체인으로 전달된다"고 단정하지 않는다.
정확한 표현은 이렇다. 검색한 애플리케이션 소스에서 명시적 읽기를 찾지 못했다. parked 워크플로가
조건부로 둘을 주입하고(`.github/workflows/repair-cluster-worker.yml.disabled:685`),
자식 환경이 `process.env` 를 그대로 물려주며(`src/repair/process-env.ts:20`) 수정 실행기가 그
환경으로 에이전트를 띄운다(`src/repair/execute-fix-artifact.ts:395`). 그 다음 소비는 확인하지 않았다.

`config/` 일곱 개도 "소비 지점이 있다"를 "운영 필수 값이다"로 바꾸지 않는다. 성격을 나눠 적는다.
런타임 한도, 문서 매니페스트, 조치 시드 데이터, 조건부 Knip 락파일이다. Knip 은 대상이
`openclaw/openclaw` 이고(`src/repair/pinned-openclaw-validation-helper.ts:34`) 러너가 있고 pin 이
지원 목록에 있을 때만 쓰인다.

### D5 · 연결점 문서의 경계 (050)

"프롬프트 하나, schema 하나"는 **항목 리뷰 경로**에 한정된다. 다른 경로가 있다
(`src/clawsweeper-review-runtime.ts:470` 의 PR 종료 근거 증빙 프롬프트). 실행은 `:1127` 이다.
...
**이 전제는 틀렸다.** PR 을 열고 나서 Devin Review 와 Codex 코드리뷰가 둘 다 설치되어 실제로
돌았다. Actions 비활성·브랜치 보호 없음은 GitHub App 리뷰와 무관하다. 정정과 관측 값은
`070_delivery_gates.md` 의 "정정: 호스티드 리뷰어가 있다" 절에 있다. 독립 리뷰는 대체물이 아니라
추가 층으로 남는다.
