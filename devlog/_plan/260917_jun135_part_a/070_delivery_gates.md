...
author·committer 가 `<계정 비공개 이메일>` 이고, 이미 원격에 있는 `c80ecac8` 은
...
1번은 `<계정 비공개 이메일>` 을 공개 저장소 이력에 영구히 남기기 때문이다. 미푸시 커밋 10개의
...
`<계정 비공개 이메일>` 이었기 때문이다. 저장소 로컬 설정을 noreply 로 바꾸고 해당 커밋을 amend 해서
...
(Devin Review · Codex) 10회전이다. 총 33건을 받아 31건을 고치고 2건을 한계·의도로 명시했다.
...
**10회전 · head `a2070a94` · 1건 (P1)**

| # | 제공자 | 등급 | 위치 | 지적 | 처리 | 재확인 |
| -- | -- | -- | -- | -- | -- | -- |
| H28 | Codex | **P1** | `070_delivery_gates.md` | 이 기록이 GitHub 이 비공개로 판정한 주소를 그대로 적고 있다. 커밋 메타데이터를 재작성해도 이 파일 때문에 공개 저장소가 그 주소를 영구히 드러낸다 | 이 커밋 | 세 곳을 `<계정 비공개 이메일>` 로 치환. 작업물 검색 결과 0건. 커밋 메시지에는 원래 없었다(`1f36c10e..HEAD` 전수 확인) |

### 남은 노출 (코디네이터 판단 필요)

이 지적이 이 유닛에서 가장 중요한 것이다. 신원 재작성의 목적 자체를 내 문서가 무효로 만들고 있었다.
저장소는 `visibility=public` 이므로 노출은 실제였다.

팁(tip)은 이 커밋으로 고쳤고 앞으로 머지되는 트리에는 주소가 없다. 그러나 **이미 push 된 중간
커밋 12개의 blob 에는 남아 있다.** 확인 명령은 다음과 같다.

```
git rev-list 1f36c10e..HEAD | while read sha; do
  git grep -q '<주소>' $sha -- devlog && echo $sha
done
```

해당 커밋은 `d5493450`부터 `a2070a94`까지 12개다.

`DEV-PRIVACY-01` 은 이런 적발을 후속 커밋이 아니라 **미push 커밋 재작성** 으로 고치라고 한다.
이 커밋들은 이미 push 됐으므로 그 방법은 force-push 를 요구한다. 그것은 공유 원격에 대한 파괴적
조작이고, 리뷰어가 본 이력을 바꾸며, 머지 권한자인 코디네이터의 결정 사항이다. 내가 임의로 하지
않았다.

선택지는 둘이다. (1) 브랜치를 재작성해 force-push 한다. (2) squash 머지를 쓴다. squash 면 `main` 에는
최종 트리만 들어가므로 주소가 들어가지 않지만, PR ref 를 통해 중간 커밋은 계속 도달 가능하다.
완전 제거를 원하면 (1) 이 필요하고, 캐시된 ref 정리를 GitHub 지원에 요청해야 할 수도 있다.
